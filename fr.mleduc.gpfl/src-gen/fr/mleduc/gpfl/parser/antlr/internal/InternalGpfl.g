...
entrySuperXLiteral returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getXbaseXLiteralRule()); }
	iv_superXLiteral=superXLiteral
	{ $current=$iv_superXLiteral.current; }
	EOF;

// Rule XLiteral
superXLiteral returns [EObject current=null]
@init {
	enterRule();
}
@after {
...
@after {
	...
	(
		...
		(
			('extends' | 'static' | 'import' | 'extension' | '!' | '-' | '+' | 'new' | '{' | 'switch' | 'synchronized' | '<' | 'super' | '#' | '[' | 'false' | 'true' | 'null' | 'typeof' | 'inSide' | 'outSide' | 'if' | 'for' | 'while' | 'do' | 'throw' | 'return' | 'try' | '(' | RULE_ID | RULE_HEX | RULE_INT | RULE_DECIMAL | RULE_STRING)=>
			(
				{
