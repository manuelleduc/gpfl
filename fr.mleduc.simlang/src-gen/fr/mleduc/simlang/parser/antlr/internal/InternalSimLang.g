...
entryRuleRootBlockExpression returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getRootBlockExpressionRule()); }
	iv_ruleRootBlockExpression=ruleRootBlockExpression
	{ $current=$iv_ruleRootBlockExpression.current; }
	EOF;

// Rule RootBlockExpression
ruleRootBlockExpression returns [EObject current=null]
@init {
	enterRule();
}
@after {
...
@after {
	...
	(
		...
		{
			$current = $this_XPrimaryExpression_7.current;
			afterParserOrEnumRuleCall();
		}
	)
;
...
@after {
	...
	(
		...
		(
			('extends' | 'static' | 'import' | 'extension' | '!' | '-' | '+' | 'cond' | 'new' | '{' | 'switch' | 'synchronized' | '<' | 'super' | '#' | '[' | 'false' | 'true' | 'null' | 'typeof' | 'if' | 'for' | 'while' | 'do' | 'throw' | 'return' | 'try' | '(' | RULE_ID | RULE_HEX | RULE_INT | RULE_DECIMAL | RULE_STRING)=>
			(
				{
